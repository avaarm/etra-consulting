...
function VideoItem({galleryItem}) {
    ...
    return (
        <div className="rn-gallery icon-center video-gallery-content bg_image" data-black-overlay="7" style={{backgroundImage: `url(${process.env.PUBLIC_URL}/images/bg/bg-image-4.mp4)`}}>
            <div className="overlay-content">
                <button className="btn-default rounded-player sm-size" onClick={ () => setToggler(!toggler) }><span>{iconHandel()}</span></button>
                <h3 className="title">Watch the process behind <br /> our work</h3>
            </div>
